...
```{r}
# housing_data$residuals <- fit$residuals
# 
# summary(housing_data[housing_data$residuals>0,-(1+q+1:pz)])
# summary(housing_data[housing_data$residuals<0,-(1+q+1:pz)])
...
df3n = filter(housing_data, housing_data$prov_label==4)
df3=apply(df3n, 2, as.numeric)
colnames(df3)=colnames(df3n)

px=0
set=mvHTP.Vhat(Y=as.matrix(df3[,1]),D=df3[,1+1:q],W=df3[,1+q+ 1:(pz+px)],intercept=FALSE,pz=pz,s=5,tuning = 3)


Zs=df3[,1+q+set$Shat]
W=cbind(Zs,df3[,1+q+pz+ 1:px])
  auxreg <-lm.fit(W, df3[,1+1:q])
 Dhat = as.matrix(auxreg$fitted.values)
res=data.frame(df3[,1+1:q] -Dhat)
colnames(res)=c("income_v","gender_v","education_v")
candidates=1+q+1:pz
colnames(df3[,1+q+set$Vhat])
drop=union(candidates[-set$Shat],candidates[set$Vhat])
#drop=candidates[-set$Vhat]
housing_res=cbind(df3[,-drop],res)
formula_invalid = write_formula(housing_res)
fit3=glm(formula_invalid,family = binomial(link = "probit"),housing_res)
summary(fit3)
