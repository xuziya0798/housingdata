...
Z <- select(data,c(father_edu_head,mother_edu_head,spouse_edu_head,prov_label, familysize18,age_head,
...
mvHTP_01(Y=as.matrix(df[,1]),D=as.matrix(df[,1+1:q]),
         Z=as.matrix(df[,1+q+ 1:pz]),X=as.matrix(df[,1+q+pz+1:px]),s=8,tuning = 3)
...
```{r}
df4n = filter(housing_data, housing_data$prov_label==4)
df4=apply(df4, 2, as.numeric)
colnames(df4)=names(df4n)

mvHTP_01(Y=as.matrix(df4[,1]),D=as.matrix(df4[,1+1:q]),
         Z=as.matrix(df4[,1+q+ 1:pz]),X=as.matrix(df4[,1+q+pz+1:px]),s=5,tuning = 5)
set=mvHTP.Vhat(Y=as.matrix(df4[,1]),D=df4[,1+1:q],W=df4[,1+q+ 1:(pz+px)],intercept=TRUE,pz=pz,s=5,tuning = 5)
colnames(df4[,1+q+set$Vhat])
colnames(df4[,1+q+set$Shat])




Zs=df4[,1+q+set$Shat]
W=cbind(Zs,df4[,1+q+pz+ 1:px])
  auxreg <-lm.fit(W, df4[,1+1:q])
 Dhat = as.matrix(auxreg$fitted.values)
res=data.frame(df4[,1+1:q] -Dhat)
colnames(res)=c("income_v","gender_v","education_v")
candidates=1+q+1:pz
colnames(df4[,1+q+set$Vhat])
drop=union(candidates[-set$Shat],candidates[set$Vhat])
#drop=candidates[-set$Vhat]
housing_res=cbind(df4[,-drop],res)
formula_invalid = write_formula(housing_res)
fit=glm(formula_invalid,family = binomial(link = "probit"),housing_res)
summary(fit)
