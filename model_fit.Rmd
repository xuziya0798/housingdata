...
set=mvHTP.Vhat(Y=as.matrix(df[,1]),D=df[,1+1:q],W=df[,-(1+(q+1))],intercept=TRUE,pz=pz,s=5,tuning = 5)
Zs=Z[,set$Shat]
  W=cbind(Zs,X)
  X_=cbind(D,Zs[,-set$Vhat],X)
  auxreg <-lm.fit(W, D)
 Dhat = as.matrix(auxreg$fitted.values)
res=data.frame( D-Dhat)
colnames(res)=c("income_v","marriage_v","education_v","children_v")
candidates=1+q+1:pz
drop=union(candidates[-Shat],candidates[Vhat])
